...
exports.compute_borders = compute_borders;

function compute_borders(buffer) {
...
var _borders2 = _interopRequireDefault(require("./shaders/borders.vert"));

var _texture = _interopRequireDefault(require("./shaders/texture.frag"));
...
async function getBorders() {
  return fetch(`./${_borders3.default}`).then(response => response.arrayBuffer()).then(buffer => (0, _vertices.compute_borders)(buffer));
}
...
async function main() {
  const [borders, landTexture, monoTexture] = await Promise.all([getBorders(), getTexture(_specularity2x.default), getTexture(_mono2x.default)]);
  const drawBorders = createLineDrawer(borders);
  const drawTexture = regl({
    frag: _texture.default,
    vert: _texture2.default,
    uniforms: {
      landTexture,
      monoTexture,
      aspectRatio,
